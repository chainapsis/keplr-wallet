...
describe('Test BinarySortArray', () => {
  test('only push sort test', async () => {
    ...
    expect(sort.arr.map(v => v.value)).toStrictEqual([1, 2, 2.1, 3, 3.1, 3.05]);
  ...
  test('sort existing key test', async () => {
    ...
    expect(sort.indexOf('3.1')).toBe(4);
    expect(sort.indexOf('3.05')).toBe(5);
    expect(sort.arr.map(v => v.value)).toStrictEqual([1.5, 2, 2.1, 3, 3.1, 3]);
    ...
    expect(sort.indexOf('3.05')).toBe(4);
    expect(sort.indexOf('2')).toBe(5);
    expect(sort.arr.map(v => v.value)).toStrictEqual([1.5, 2.1, 3, 3.1, 3, 4]);
    ...
    expect(sort.arr.map(v => v.value)).toStrictEqual([2.1, 3, 3.1, 3.5, 4, 5]);
  });
