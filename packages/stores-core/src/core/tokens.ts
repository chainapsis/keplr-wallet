...
export class TokensStore {
  ...
  async addToken(chainId: string, currency: AppCurrency): Promise<void> {
    ...
    if ("cosmos" in modularChainInfo) {
      ...
      runInAction(() => {
        ...
        this.updateTokenMap(chainIdentifier, res[chainIdentifier]);
      });
    } else if ("evm" in modularChainInfo || "starknet" in modularChainInfo) {
  ...
  private updateTokenMap(chainIdentifier: string, newTokens?: TokenInfo[]) {
    ...
    if (newTokens) {
      newTokenMap.set(chainIdentifier, newTokens);
    }
