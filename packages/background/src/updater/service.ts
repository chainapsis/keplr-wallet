      ...
      try {
        ...
        const currencies = chainInfo.currencies.map((currency: AppCurrency) => {
          const hasCurrency = originChainInfo.currencies.find(
            (originCurrency) =>
              originCurrency.coinMinimalDenom === currency.coinMinimalDenom
          );

          return { ...currency, coinImageUrl: hasCurrency?.coinImageUrl };
        });
        const feeCurrencies = chainInfo.feeCurrencies.map(
          (currency: AppCurrency) => {
            const hasCurrency = originChainInfo.feeCurrencies.find(
              (originCurrency) =>
                originCurrency.coinMinimalDenom === currency.coinMinimalDenom
            );
            return { ...currency, coinImageUrl: hasCurrency?.coinImageUrl };
